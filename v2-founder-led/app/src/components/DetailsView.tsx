import { AlertCircle, Anchor, ExternalLink, RefreshCw, Ruler, ShieldCheck } from "lucide-react";
import type { Variant } from "../../shared/types";
import { euro } from "../utils";

export function DetailsView({ variant, busy, onSwap }: { variant?: Variant; busy: boolean; onSwap: (variantId: string) => Promise<void> }) {
  ...
  return <div className="details-view">
    ...
    {variant.category !== "attachment" && <button className="quiet-button product-swap" type="button" disabled={busy} onClick={() => onSwap(variant.variantId)}><RefreshCw className={busy ? "spin" : ""} size={16} />Try another checked {variant.category.replaceAll("_", " ")}</button>}
